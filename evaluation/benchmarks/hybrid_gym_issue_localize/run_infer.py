...
def to_json_safe_dict(series: pd.Series):
    ...
    for k, v in series.items():
        # Try to convert ndarray to list
...
def _get_swebench_workspace_dir_name(instance: pd.Series) -> str:
    version = instance.version if hasattr(instance, 'version') and instance.version is not None else instance.pull_number
...
Phase 4. COMMENT GENERATION: left a comment at each relevant location to explain why they are related to the issue
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with open(temp_file_path, 'w') as f:
            if not isinstance(instance, dict):
                json.dump([to_json_safe_dict(instance)], f)
            else:
...
def process_instance(
    ...
    output = EvalOutput(
        ...
        instance=to_json_safe_dict(instance),  # SWE Bench specific
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--filter_instances_from_files',
        type=str,
        default="",
        help='jsonl files to filter instances from',
    )
    parser.add_argument(
        '--select_instances_from_files',
        type=str,
        default="",
        help='json files to select instances from',
    )
    ...
    if args.filter_instances_from_files:
        print(f"Dataset size before filtering: {len(dataset)}")
        for filter_instances_from_file in args.filter_instances_from_files.split(','):
            with open(filter_instances_from_file, 'r') as f:
                filtered_instances = [json.loads(line)['instance_id'] for line in f]

            dataset = dataset.filter(lambda x: x['instance_id'] not in set(filtered_instances))

        print(f"Dataset size after filtering: {len(dataset)}")

    if args.select_instances_from_files:
        print(f"Dataset size before selecting: {len(dataset)}")
        selected_instances = []
        for select_instances_from_file in args.select_instances_from_files.split(','):
            with open(select_instances_from_file, 'r') as f:
                selected_instances.extend(json.load(f))

        dataset = dataset.filter(lambda x: x['instance_id'] in set(selected_instances))

        print(f"Size of Selected instances: {len(selected_instances)}")
        print(f"Dataset size after selecting: {len(dataset)}")
    ...
    #     orig_model_name = llm_config.model.split('/')[-1]
    #     orig_model_path = model_path = orig_model_name.replace(':', '_').replace('@', '-')

    #     new_model_name = f'{args.save_model_name}'
    #     new_model_path = new_model_name.replace(':', '_').replace('@', '-')

    #     metadata.eval_output_dir = metadata.eval_output_dir.replace(orig_model_path, new_model_path)
